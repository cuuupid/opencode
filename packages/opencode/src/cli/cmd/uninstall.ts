...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall iris and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# iris") || content.includes(".iris/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".iris/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".iris/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".iris"))
    ) {
      continue
    }
