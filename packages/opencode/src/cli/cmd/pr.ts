  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        const irisArgs = sessionId ? ["-s", sessionId] : []
        const irisProcess = Process.spawn(["iris", ...irisArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await irisProcess.exited
        if (code !== 0) throw new Error(`iris exited with code ${code}`)
      },
    })
  },
})
